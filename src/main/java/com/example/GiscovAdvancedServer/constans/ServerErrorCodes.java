...
public enum ServerErrorCodes {
    ...
    NO_ACCESS_TO_NEWS(48, ValidationConstants.NO_ACCESS_TO_NEWS);

    private final Integer code;

    private final String massage;

    public static Map<String, Integer> mapError;

    static {
        mapError = new HashMap<>();
        for (ServerErrorCodes sec : ServerErrorCodes.values()) {
            mapError.put(sec.massage, sec.code);
        }
    }
}
